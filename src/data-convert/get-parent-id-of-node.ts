...
export const getParentIdOfNode = ( { node, activeAntiPatterns }: {
  ...
  const parentId = Object.keys(activeAntiPatterns).sort().reduce(( acc, antipatternName ) => {
    if ( node.id in activeAntiPatterns[antipatternName].metric || Object.values(activeAntiPatterns[antipatternName].topological).flat().flat().flat().flat().includes(node.id) ) {
      if ( acc ) {
